...
fn gen_py_struct(mut gen_ctx: &mut GenContext, s: &Struct) -> Vec<PyToken> {
    ...
    body.push(gen_typevar_dump_fields(
        &s.type_parameters
            .iter()
            .map(|tp| PyToken::Raw(tp.clone()))
            .collect(),
    ));
    ...
    from_json_args.extend(s.type_parameters.iter().map(|tv| {
        PyToken::Block(vec![
            ...
            gen_generic_parse_dump_hint(&tv.clone().into()),
        ])
    }));
...
fn gen_generic_parse_dump_hint(tv: &PyToken) -> PyToken {
    let mut tokens = Vec::new();
...
fn gen_alias_struct(gen_ctx: &GenContext, a: &Alias, r: &RefType, s: &Struct) -> Vec<PyToken> {
    let mut superclass_toks = vec![s.name.clone().into()];

    if !r.type_parameters.is_empty() {
        superclass_toks.push("[".into());
        let type_vars = r
            .type_parameters
            .iter()
            .map(|t| PyToken::Block(t.get_type_vars(&gen_ctx, TypeVarKind::AllTypes)))
            .collect::<Vec<_>>();

        superclass_toks.extend(intersperce(", ".into(), type_vars));
        superclass_toks.push("]".into());
    }
    ...
    py_class(&a.name, vec![PyToken::Block(superclass_toks)], body)
}
...
fn gen_py_sum_type(mut gen_ctx: &mut GenContext, e: &Enum) -> PyToken {
    ...
    tokens.push(gen_py_sum_typedef(&gen_ctx, &e));
    tokens.push(PyToken::NewLine);
    tokens.push(PyToken::NewLine);
...
fn gen_py_sum_typedef(ctx: &GenContext, e: &Enum) -> PyToken {
    let mut tokens = vec![];
    ...
    tokens.push(typed_union(
        e.variants
            ...
            .map(|v| {
                ...
                let tvs = v.value.get_type_vars(&ctx, TypeVarKind::OnlyTypeVars);
                if !tvs.is_empty() {
                    toks.push("[".into());
                    toks.extend(intersperce(", ".into(), tvs));
...
fn gen_from_json_enum(gen_ctx: &GenContext, e: &Enum) -> PyToken {
    let mut from_json_body = vec![];
        ...
        from_json_body.push(
            If::new(
                ...
                if arg.is_empty() {
                    ...
                    let fn_name = if variant
                        .value
                        .get_type_vars(&gen_ctx, TypeVarKind::OnlyTypeVars)
                        .is_empty()
                    {
                        "from_json"
                    } else {
    ...
    match json_directive.repr {
        ...
        JsonRepr::Union => {
            ...
            for v in other_ctors.iter() {
                ...
                let tvs = v.value.get_type_vars(&gen_ctx, TypeVarKind::OnlyTypeVars);
                if tvs.is_empty() {
                    from_json_body.push(
                    ...
                    for tv in tvs {
                        ...
                        from_json_body.push("parse_".into());
                        from_json_body.push(tv);
                        from_json_body.push(", ".into());
                    }
...
fn gen_typevar_dump_fields(type_vars: &Vec<PyToken>) -> PyToken {
    PyToken::Block(intersperce(
        PyToken::NewLine,
        type_vars
            .iter()
            .map(|tv| {
                let tv_name = match tv {
                    PyToken::Raw(x) => Some(x),
                    _ => None,
                };

                typed_attr(
                    // TODO figure out how to pass the name of the typevar alongside the token
                    &format!(
                        "_dump_{}",
                        tv_name.expect("typevars should be simple token")
                    ),
                    vec![PyToken::Block(vec![
...
fn gen_enum_variant_typedict(
    ...
    match &variant.value {
        ...
        VariantValue::PositionalCtor(refs) => {
            ...
            if refs.len() == 1 {
                td_class_body.push(typed_attr(&content_name, vec![type_import("Any")], None));
            } else {
                let mut typs = vec![type_import("Tuple")];
                typs.push("[".into());
                typs.extend(intersperce(
                    ", ".into(),
                    refs.iter().map(|_| type_import("Any")).collect(),
                ));
                typs.push("]".into());
...
fn gen_enum_variant_tuple(_gen_ctx: &mut GenContext, variant: &EnumVariant) -> (String, PyToken) {
    ...
    match &variant.value {
        ...
        VariantValue::PositionalCtor(refs) => {
            for _r in refs {
                ...
                tuple_args.push(type_import("Any"));
            }
        }
    }
...
fn gen_enum_variant_union(_gen_ctx: &mut GenContext, variant: &EnumVariant) -> (String, PyToken) {
    match &variant.value {
        ...
        VariantValue::PositionalCtor(refs) => {
            ...
            if refs.len() == 1 {
                return (
                    name.clone(),
                    PyToken::Block(vec![format!("{} = ", name).into(), type_import("Any")]),
                );
            }
...
fn gen_enum_variant_from_json(
    ...
    let return_type = if tvs.is_empty() {
        ...
        let mut rt = vec![format!("\"{}[", variant.name.to_pascal_case()).into()];
        rt.extend(intersperce(", ".into(), tvs));
        rt.push("]\"".into());
        Some(PyToken::Block(rt))
    };
    ...
    match &variant.value {
        ...
        VariantValue::PositionalCtor(refs) => {
            if refs.len() == 1 {
                ...
                from_json_body.push(PyToken::NewLine);
                from_json_body.push(gen_typevar_dump_fields(
                    &variant
                        .value
                        .get_type_vars(&gen_ctx, TypeVarKind::OnlyTypeVars),
                ));
                ...
                cls_args.append(
                    &mut variant
                        ...
                        .get_type_vars(&gen_ctx, TypeVarKind::OnlyTypeVars)
                        .into_iter()
                        .map(|tv| PyToken::Block(vec!["dump_".into(), tv]))
                        .collect(),
                );
...
// flag to chose wether to return atomic types along with generic type variables
...
impl Type {
    fn get_type_vars(&self, ctx: &GenContext, typevar_kind: TypeVarKind) -> Vec<PyToken> {
        match &self {
            Type::Atomic(a) => match typevar_kind {
                TypeVarKind::OnlyTypeVars => vec![],
                TypeVarKind::AllTypes => vec![atomic_py_instance(&a).0.into()],
            },
            Type::Reference(r) => match typevar_kind {
                TypeVarKind::OnlyTypeVars => r
                    .type_parameters
                    .iter()
                    .map(|t| t.get_type_vars(&ctx, typevar_kind))
                    .flatten()
                    .collect(),
                TypeVarKind::AllTypes => {
                    vec![r.get_python_type(&ctx)]
                }
            },
            Type::Builtin(b) => match b {
                Builtin::List(t) => t.get_type_vars(&ctx, typevar_kind),
                Builtin::Optional(o) => o.get_type_vars(&ctx, typevar_kind),
                Builtin::Map(k, v) => {
                    let mut ks = k.get_type_vars(&ctx, typevar_kind);
                    let mut vs = v.get_type_vars(&ctx, typevar_kind);
                    ks.append(&mut vs);
                    ks
                }
            },
            Type::TypeParameter(t) => vec![t.clone().into()],
        }
    }
...
impl VariantValue {
    fn get_type_vars(&self, ctx: &GenContext, k: TypeVarKind) -> Vec<PyToken> {
        match &self {
            VariantValue::OnlyCtor => Vec::new(),
            VariantValue::PositionalCtor(typs) => typs
                .iter()
                .map(|t| t.get_type_vars(&ctx, k))
                .flatten()
                .collect(),
        }
    }
