...
fn main() -> Result<()> {
    ...
    let mut dest_path: PathBuf = opt.target_dir.unwrap_or(dest_path_default);
    ...
    let mut dest = File::create(&dest_path)
        .with_context(|| format!("Cannot create target file at: {:?}", dest_path))?;
    println!("writing result to {:?}", dest_path);
    dest.write_all("# File generated by the dare compiler, do not manually edit !\n\n".as_bytes())
        .with_context(|| format!("Cannot write generated code to {:?}", dest))?;
...
mod test {
    use super::*;
    use ast::{
        Alias, AtomicType, Builtin, Directives, Enum, EnumVariant, Field, FieldType, JsonDirective,
        JsonRepr, RefType, Struct, Type, VariantValue,
    };
